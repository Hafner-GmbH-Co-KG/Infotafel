...
def _resolve_monitor(identifier: Optional[str]) -> Optional[Monitor]:
...
def _base_layers_for_monitor(monitor: Optional[Monitor]) -> Dict[str, bool]:
...
def _apply_specific_overrides(
    ...
    display_state: Optional[DisplayState],
    ...
    if monitor and monitor.pk and content_item_id:
        override = (
            MonitorContentOverride.objects.filter(
                monitor=monitor, content_item_id=content_item_id
            )
            .order_by("-updated_at")
            .first()
        )
...
def _voices_from_payload(payload: Optional[Dict[str, Any]]) -> Dict[str, bool]:
...
def _build_content_from_state(
    ...
    if item.item_type == ContentItemType.BIBLE and hasattr(item, "bible_reference"):
        ...
        bible_reference_payload = {
            ...
            "reference": _format_bible_reference(reference.book, reference.chapter, reference.verse_start, reference.verse_end),
            "text": reference.text,
        }
        if not text:
...
def resolve_display_for_monitor(identifier: Optional[str] = None) -> DisplayContext:
